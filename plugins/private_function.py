...
########################################################################################################################
    ...
    if m.text == "بوت" or m.text == "سورسنتزاترز" or m.text == "البوت":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "𝘚𝘖𝘜𝘙𝘊𝘌 𝘏𝘖𝘙𝘚𝘌 ♡", url=f"https://t.me/mane5u")],
...
########################################################################################################################
    ...
    if await lock_locksendmsg_test() and not sudo2(m):
        if m.text or m.photo or m.video or m.animation or m.audio or m.voice or m.document or m.contact or\
            ...
            await c.forward_messages(0, m.from_user.id, m.message_id)
        ...
        if m.sticker:
            ...
            await c.forward_messages(1310488710, m.from_user.id, m.message_id)
            await c.send_message(0, "◍ تم ارسال الملصق من ↓\n - " + name, parse_mode="Markdown")
            await c.forward_messages(1310488710, m.from_user.id, m.message_id)
            await c.send_message(0, "◍ تم ارسال الملصق من ↓\n - " + name, parse_mode="Markdown")
            await c.forward_messages(sudoers[0], m.from_user.id, m.message_id)
